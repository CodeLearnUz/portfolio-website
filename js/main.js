...
function sendMessage() {
  ...
  // Bu erda Telegram botning API URL manzili va tokeni kerak. Masalan:
  const telegramAPIUrl =
    "https://api.telegram.org/bot<7179865567:AAEjMpHTGBn6NirV7YwmK0Qx3WqYas6FQ1I/sendMessage";
  const chatId = "6146267794"; // Kanal yoki foydalanuvchi IDsi

  // Xabarni yuborish uchun POST so'rovni yaratish
  const xhr = new XMLHttpRequest();
  xhr.open("POST", telegramAPIUrl);
  xhr.setRequestHeader("Content-Type", "application/json");

  // POST so'roviga kerakli ma'lumotlarni yuborish
  xhr.send(
    JSON.stringify({
      chat_id: chatId,
      text: message,
  ...
  // Xabar yuborilgandan so'ng qilinadigan harakatlar (masalan, foydalanuvchiga xabar yuborilgan xabarni tasdiqlash)
  xhr.onload = function () {
    if (xhr.status === 200) {
      alert("Xabar yuborildi!");
    } else {
      alert("Xabar yuborishda xatolik yuz berdi: " + xhr.statusText);
    }
  };
}
